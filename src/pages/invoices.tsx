...
export default function Invoices(): JSX.Element {
  ...
  const currentEmpolyee: currentEmpolyeeType = {
    name: JSON.parse(localStorage.getItem('employee_data') || '').full_name ?? null,
    id: Number(JSON.parse(localStorage.getItem('employee_data') || '').id) ?? null,
    role: JSON.parse(localStorage.getItem('employee_data') || '').role ?? 'employee'
  }
  ...
  const calculateTotalRevenue = () => {
    ...
    const filteredReceipts = filterReceipts(
      receipts.filter(
        receipt =>
          receipt.employee_id === currentEmpolyee.id || currentEmpolyee.role === 'admin'
      )
    )
  ...
  }, [receipts, allOfficeDetails, searchTerm, currentEmpolyee.id, currentEmpolyee.role])
  ...
  const handleCheckboxChange = (receiptId: number): void => {
    ...
    const selected: receiptsType[] = updatedReceipts.filter(receipt => receipt.selected)
    setSelectedReceipts(selected)
  }

  const filterReceipts = (receipts: receiptsType[]): receiptsType[] => {
    return receipts.filter(receipt => {
      const matchDate: boolean = arabicDate(receipt.created_at).includes(searchTerm)
